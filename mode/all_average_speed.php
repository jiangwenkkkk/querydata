...
class all_average_speed extends query_mode
{
    ...
    {
        $this->data["all"] = ['speed' => $row["average_speed"]];
    }
...
SELECT
  max(E.output_bill_count / E.distri_t),
  min(E.output_bill_count / E.distri_t),
  avg(E.output_bill_count / E.distri_t) AS average_speed
FROM
  (SELECT
     D.input_file,
     D.output_bill_count,
      ...
      FROM
        (SELECT
         ...
         WHERE log_type = 1
               AND module_id = 1
               AND log_time >= UNIX_TIMESTAMP('$begin')
               AND log_time < UNIX_TIMESTAMP('$end')) A
        INNER JOIN
        (SELECT
           input_file,
           Handlestattime,
         ...
         WHERE log_type = 3
               AND module_id = 1
               AND log_time >= UNIX_TIMESTAMP('$begin')
               AND log_time < UNIX_TIMESTAMP('$end')) B
          ON
